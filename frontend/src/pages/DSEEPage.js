  ...
  return (
    <DashboardLayout>
      ...
      <div className="space-y-4 max-w-[900px]" style={{ fontFamily: fontFamily.body }} data-testid="dsee-page">
        <div>
          <h1 className="text-2xl font-semibold text-[var(--ink-display)] mb-1" style={{ fontFamily: fontFamily.display }}>Structural Exposure Analysis</h1>
          ...
          <p className="text-xs mb-3" style={{ color: 'var(--ink-muted)', fontFamily: fontFamily.mono }}>
        ...
        {running && (
          <div className="rounded-2xl p-8" style={{ background: 'var(--biqc-bg-card)', border: '1px solid var(--biqc-border)' }}>
            <div className="flex items-center gap-3 mb-3">
              ...
              <p className="text-sm text-[var(--ink-display)]" style={{ fontFamily: fontFamily.display }}>Running structural exposure analysis…</p>
            </div>
            {/* Progress stages */}
            {[
              ...
              <div key={i} className="flex items-center gap-2 mt-1.5">
                ...
                <p className="text-xs" style={{ color: stage.done ? '#10B981' : 'var(--ink-secondary, #8FA0B8)', fontFamily: fontFamily.mono }}>{stage.label}</p>
              </div>
            ))}
          </div>
        )}
        ...
        {r && !running && <>
          ...
          <div className="rounded-2xl p-5" style={{ background: '#E85D0008', border: '1px solid #E85D0025' }} data-testid="dsee-verdict">
            <div className="flex items-start justify-between gap-4 flex-wrap">
              <div>
                ...
                <h2 className="text-xl font-semibold text-[var(--ink-display)]" style={{ fontFamily: fontFamily.display }}>{r.business_name}</h2>
                <div className="flex items-center gap-3 mt-1">
                  {r.location && <span className="text-xs text-[var(--ink-secondary)] flex items-center gap-1"><MapPin className="w-3 h-3" />{r.location}</span>}
                  <span className="text-[10px] text-[var(--ink-muted)]" style={{ fontFamily: fontFamily.mono }}>{r.domain?.resolved_domain}</span>
                </div>
              </div>
              <div className="text-right">
                ...
                <span className="text-[10px] text-[var(--ink-muted)]" style={{ fontFamily: fontFamily.mono }}>confidence{conf.confidence_cap_applied ? ' (capped)' : ''}</span>
              </div>
            </div>
            <div className="flex flex-wrap gap-3 mt-3">
              ...
              <div className="px-3 py-1.5 rounded-lg" style={{ background: 'var(--biqc-bg)', border: '1px solid var(--biqc-border)' }}>
                ...
                <span className="text-lg font-bold text-[var(--ink-display)]" style={{ fontFamily: fontFamily.mono }}>{r.competitor_count}</span>
              </div>
              <div className="px-3 py-1.5 rounded-lg" style={{ background: 'var(--biqc-bg)', border: '1px solid var(--biqc-border)' }}>
                <span className="text-[10px] text-[var(--ink-muted)] block" style={{ fontFamily: fontFamily.mono }}>Reviews</span>
                <span className="text-lg font-bold text-[var(--ink-display)]" style={{ fontFamily: fontFamily.mono }}>{r.reviews?.google_reviews || 0}</span>
              </div>
              <div className="px-3 py-1.5 rounded-lg" style={{ background: 'var(--biqc-bg)', border: '1px solid var(--biqc-border)' }}>
                <span className="text-[10px] text-[var(--ink-muted)] block" style={{ fontFamily: fontFamily.mono }}>Search</span>
          ...
          <div className="rounded-2xl overflow-hidden" style={{ background: 'var(--biqc-bg-card)', border: '1px solid var(--biqc-border)' }} data-testid="dsee-asymmetries">
            <button onClick={() => toggle('asym')} className="w-full flex items-center justify-between p-4 hover:bg-white/[0.02]">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-semibold text-[var(--ink-display)]" style={{ fontFamily: fontFamily.display }}>Structural Exposures ({r.asymmetry_count})</span>
              </div>
              {sections.asym ? <ChevronUp className="w-4 h-4 text-[var(--ink-muted)]" /> : <ChevronDown className="w-4 h-4 text-[var(--ink-muted)]" />}
            </button>
            {sections.asym && (
                  ...
                  return (
                    <div key={i} className="rounded-xl p-4" style={{ background: tc + '06', border: `1px solid ${tc}20` }}>
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-[10px] ml-auto" style={{ color: 'var(--ink-muted)', fontFamily: fontFamily.mono }}>{Math.round((a.confidence || 0) * 100)}%</span>
                      </div>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                        <div className="p-2 rounded" style={{ background: 'var(--biqc-bg)' }}>
                          ...
                          <span className="text-[var(--ink-secondary)]">{a.subject_metric}</span>
                        </div>
                        <div className="p-2 rounded" style={{ background: 'var(--biqc-bg)' }}>
                          <span className="text-[10px] text-[var(--ink-muted)] block mb-0.5" style={{ fontFamily: fontFamily.mono }}>Competitor</span>
                          <span className="text-[var(--ink-display)]">{a.competitor_metric}</span>
                        </div>
                      </div>
                      <div className="flex items-center gap-3 mt-2 text-[10px]" style={{ fontFamily: fontFamily.mono }}>
                        <span className="text-[var(--ink-muted)]">Ratio: <strong className="text-[var(--ink-display)]">{a.differential_ratio}</strong></span>
                        <span className="text-[var(--ink-muted)]">Source: {a.metric_source}</span>
                      </div>
                    </div>
                  );
                })}
            ...
            <div className="rounded-2xl overflow-hidden" style={{ background: 'var(--biqc-bg-card)', border: '1px solid var(--biqc-border)' }} data-testid="dsee-sdd">
              <button onClick={() => toggle('sdd')} className="w-full flex items-center justify-between p-4 hover:bg-white/[0.02]">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-semibold text-[var(--ink-display)]" style={{ fontFamily: fontFamily.display }}>Search Dominance Density</span>
                ...
                {sections.sdd ? <ChevronUp className="w-4 h-4 text-[var(--ink-muted)]" /> : <ChevronDown className="w-4 h-4 text-[var(--ink-muted)]" />}
              </button>
              {sections.sdd && (
                <div className="px-4 pb-4 space-y-3">
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                    {Object.entries(sdd.sdd_score?.ratios || {}).map(([k, v]) => (
                      <div key={k} className="p-3 rounded-lg text-center" style={{ background: 'var(--biqc-bg)', border: '1px solid var(--biqc-border)' }}>
                        <span className="text-[10px] text-[var(--ink-muted)] block" style={{ fontFamily: fontFamily.mono }}>{k.replace(/_/g, ' ')}</span>
                  ...
                  {sdd.normalization && (
                    <div className="p-3 rounded-lg" style={{ background: 'var(--biqc-bg)', border: '1px solid var(--biqc-border)' }}>
                      <span className="text-[10px] text-[var(--ink-muted)] block mb-1" style={{ fontFamily: fontFamily.mono }}>Normalization</span>
                      <span className="text-xs text-[var(--ink-secondary)]">Volume factor: {sdd.normalization.volume_adjustment_factor} | Pages: {sdd.normalization.pages_crawled_subject} vs avg {sdd.normalization.pages_crawled_competitor_avg} | Cap: {sdd.normalization.per_page_cap}/page | Boilerplate: suppressed</span>
                    </div>
                  )}
                </div>
              )}
          ...
          <div className="rounded-2xl overflow-hidden" style={{ background: 'var(--biqc-bg-card)', border: '1px solid var(--biqc-border)' }} data-testid="dsee-confidence">
            <button onClick={() => toggle('conf')} className="w-full flex items-center justify-between p-4 hover:bg-white/[0.02]">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-semibold text-[var(--ink-display)]" style={{ fontFamily: fontFamily.display }}>Confidence Decomposition</span>
              </div>
              {sections.conf ? <ChevronUp className="w-4 h-4 text-[var(--ink-muted)]" /> : <ChevronDown className="w-4 h-4 text-[var(--ink-muted)]" />}
            </button>
            {sections.conf && (
                ...
                {conf.penalties_applied && Object.keys(conf.penalties_applied).length > 0 && (
                  <div className="mt-2 p-2 rounded" style={{ background: '#EF444408', border: '1px solid #EF444415' }}>
                    ...
                    {Object.entries(conf.penalties_applied).map(([k, v]) => (
                      <span key={k} className="text-[10px] text-[var(--ink-secondary)] block">{k.replace(/_/g, ' ')}: {v}</span>
                    ))}
                  </div>
                )}
                <div className="p-2 rounded" style={{ background: 'var(--biqc-bg)' }}>
                  <span className="text-[10px] text-[var(--ink-muted)]" style={{ fontFamily: fontFamily.mono }}>Scope: {conf.scope_coverage?.pages_crawled}/{conf.scope_coverage?.estimated_total_pages} pages ({Math.round((conf.scope_coverage?.coverage_ratio || 0) * 100)}%) | Fallback: {conf.fallback_used ? 'yes' : 'no'} | Cap: {conf.confidence_cap_applied ? '70% applied' : 'not applied'}</span>
                </div>
              </div>
            )}
          </div>
          ...
          {/* ═══ COMPETITORS ═══ */}
          <div className="rounded-2xl p-4" style={{ background: 'var(--biqc-bg-card)', border: '1px solid var(--biqc-border)' }}>
            <span className="text-[10px] text-[var(--ink-muted)] block mb-2" style={{ fontFamily: fontFamily.mono }}>Competitors Identified</span>
            {(r.competitors || []).map((c, i) => (
              <div key={i} className="flex items-center gap-2 py-1.5" style={{ borderBottom: i < r.competitors.length - 1 ? '1px solid rgba(140,170,210,0.15)' : 'none' }}>
                <span className="text-xs text-[var(--ink-display)]">{c.name}</span>
          ...
          <span className="text-[10px] text-[var(--ink-muted)] block text-center" style={{ fontFamily: fontFamily.mono }}>{r.execution_time_ms}ms | scan:{r.scan_id}</span>
        </>}
      </div>
      </UpgradeCardsGate>
    </DashboardLayout>
