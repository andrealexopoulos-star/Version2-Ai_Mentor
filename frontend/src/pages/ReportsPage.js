      ...
      <div className="space-y-6 max-w-[1200px]" style={{ fontFamily: fontFamily.body }} data-testid="reports-page">
            ...
            <div className="space-y-2">
              {advisoryMemos.slice(0, 5).map((memo) => (
                <div key={memo.id} className="rounded-lg p-3" style={{ background: 'var(--biqc-bg)', border: '1px solid var(--biqc-border)' }}>
                  ...
                  <p className="text-xs text-[var(--ink-secondary)] mb-2">{memo.summary}</p>
                  {Array.isArray(memo.action_items) && memo.action_items.length > 0 && (
                    <div className="space-y-1">
                      {memo.action_items.slice(0, 3).map((item, idx) => (
                        <p key={`${memo.id}-${idx}`} className="text-[11px] text-[var(--ink-display)]">- {item}</p>
                      ))}
                    </div>
                  )}
                </div>
        ...
        <Panel data-testid="market-insights-report-panel">
          ...
          {marketInsightsReport ? (
            ...
            <p className="text-xs text-[var(--ink-muted)]">No Market Insights report available yet. Run calibration to generate the CMO report.</p>
          )}
        </Panel>
          ...
          <>
            ...
            {hasEvents ? (
              ...
              <Panel className="text-center py-6">
                <Shield className="w-6 h-6 text-[var(--ink-muted)] mx-auto mb-2" />
                <p className="text-xs text-[var(--ink-muted)]">No verified signal events yet. Events will appear as integrations sync data.</p>
              </Panel>
            )}
