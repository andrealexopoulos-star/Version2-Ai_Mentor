...
const HealthCapsule = ({ label, value, status, icon: Icon }) => {
  const colors = { good: '#10B981', warning: '#F59E0B', critical: '#FF6A00' };
  const c = colors[status] || colors.good;
  return (
    <div className="flex items-center gap-3 px-4 py-3 rounded-lg" style={{ background: '#0F1720', border: '1px solid #243140' }} data-testid={`health-${label.toLowerCase().replace(/\s/g,'-')}`}>
...
const AttentionCard = ({ severity, title, impact, action, actions }) => {
  ...
  const sevColors = { critical: '#FF6A00', moderate: '#F59E0B', info: '#3B82F6' };
  const c = sevColors[severity] || sevColors.info;
  return (
    <div className="rounded-lg overflow-hidden" style={{ background: '#141C26', border: `1px solid ${c}30` }} data-testid="attention-card">
          ...
          <Panel>
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                { label: 'Alerts (30d)', value: '23', color: '#FF6A00' },
