...
const AlertItem = ({ alert }) => {
  ...
  return (
    <div className="rounded-lg overflow-hidden" style={{ background: 'var(--surface, #FFFFFF)', border: `1px solid ${s.color}20` }}>
      ...
      <div className="space-y-6 max-w-[900px]">
        ...
        <div className="flex gap-4">
          {[{ label: 'Critical', count: counts.critical, color: '#E85D00' }, { label: 'Moderate', count: counts.moderate, color: '#F59E0B' }, { label: 'Info', count: counts.info, color: '#3B82F6' }].map(s => (
            <div key={s.label} className="flex items-center gap-2 px-4 py-2.5 rounded-lg" style={{ background: 'var(--surface, #FFFFFF)', border: '1px solid rgba(140,170,210,0.15)' }}>
