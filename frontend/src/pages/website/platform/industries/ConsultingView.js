        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <Panel>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-semibold text-[#F4F7FA]" style={{ fontFamily: SORA }}>Utilisation Snapshot</h3>
              <span className="text-xs px-2 py-0.5 rounded" style={{ fontFamily: MONO, color: '#FF6A00', background: '#FF6A0015' }}>Below target</span>
            </div>
            <div className="grid grid-cols-3 gap-3 mb-4">
              <MetricCard label="Current Rate" value="68%" sub="Target: 80%" color="#FF6A00" alert />
              <MetricCard label="Revenue Impact" value="-$18,000" sub="This month" color="#FF6A00" alert />
            ...
            {[
              ...
              <HeatBar key={t.name} label={t.name} value={t.util} max={100} color={t.util >= t.target ? '#10B981' : t.util >= t.target * 0.8 ? '#F59E0B' : '#FF6A00'} alert={t.util < t.target * 0.8} />
            ))}
          </Panel>
          ...
          <Panel>
            ...
            {[
              ...
              return (
                <div key={p.project} className="mb-3 p-3 rounded-lg" style={{ background: '#0F1720', border: '1px solid #243140' }}>
                  <div className="flex justify-between mb-1">
                    <span className="text-xs text-[#F4F7FA]" style={{ fontFamily: SORA }}>{p.project}</span>
          ...
          <Panel>
            <h3 className="text-sm font-semibold text-[#F4F7FA] mb-4" style={{ fontFamily: SORA }}>Proposal Stall Detector</h3>
            <div className="grid grid-cols-3 gap-3 mb-4">
              <MetricCard label="Stalled Proposals" value="3" sub="> 45 days" color="#FF6A00" alert />
              <MetricCard label="Avg Close Cycle" value="32 days" color="#F4F7FA" />
              <MetricCard label="Revenue Gap Risk" value="61%" sub="Probability" color="#FF6A00" alert />
            </div>
            {[
              ...
              <div key={p.name} className="flex items-center justify-between p-2.5 rounded mb-2" style={{ background: '#0F1720', border: '1px solid #FF6A0020' }}>
                <div>
                  <span className="text-xs text-[#F4F7FA]" style={{ fontFamily: SORA }}>{p.name}</span>
                  <span className="text-[10px] text-[#64748B] block" style={{ fontFamily: MONO }}>{p.stage} &middot; {p.days} days</span>
                </div>
                <span className="text-xs font-semibold text-[#F4F7FA]" style={{ fontFamily: MONO }}>{p.value}</span>
              </div>
            ))}
          </Panel>
        </div>
