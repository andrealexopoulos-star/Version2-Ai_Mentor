    ...
    <div className="space-y-6 max-w-[1200px]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <Panel>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-semibold text-[#F4F7FA]" style={{ fontFamily: SORA }}>Renewal Exposure Radar</h3>
              <span className="text-xs px-2 py-0.5 rounded" style={{ fontFamily: MONO, color: '#FF6A00', background: '#FF6A0015' }}>$276K at risk</span>
            </div>
            <div className="space-y-3">
              {[
                ...
                return (
                  <div key={c.client} className="flex items-center gap-4 p-3 rounded-lg" style={{ background: '#0F1720', border: '1px solid #243140' }}>
                    <div className="w-2 h-2 rounded-full" style={{ background: rc[c.risk], boxShadow: c.risk === 'high' ? `0 0 8px ${rc[c.risk]}50` : 'none' }} />
                    <div className="flex-1">
                      <span className="text-sm font-medium text-[#F4F7FA] block" style={{ fontFamily: SORA }}>{c.client}</span>
                      <span className="text-[10px] text-[#64748B]" style={{ fontFamily: MONO }}>Intervention window: {c.window}</span>
                    </div>
                    <div className="text-right">
                      <span className="text-sm font-semibold text-[#F4F7FA] block" style={{ fontFamily: MONO }}>{c.value}</span>
            ...
            <div className="mt-3 pt-3 flex justify-between" style={{ borderTop: '1px solid #243140' }}>
              ...
              <span className="text-sm font-bold text-[#FF6A00]" style={{ fontFamily: MONO }}>$276,000</span>
            </div>
          </Panel>
          ...
          <Panel>
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              <MetricCard label="Top Client" value="34%" sub="Meridian Group" color="#FF6A00" alert />
              <MetricCard label="Top 3 Clients" value="61%" sub="$496K of $812K revenue" color="#F59E0B" alert />
            </div>
            <div className="p-3 rounded-lg" style={{ background: '#0F1720', border: '1px solid #FF6A0020' }}>
              <span className="text-[10px] text-[#64748B] uppercase" style={{ fontFamily: MONO }}>If Top Client Lost</span>
              <div className="flex items-center gap-4 mt-1">
                <span className="text-sm text-[#FF6A00]" style={{ fontFamily: MONO }}>Revenue: -34%</span>
                <span className="text-sm text-[#FF6A00]" style={{ fontFamily: MONO }}>Runway: -2.1 months</span>
              </div>
            </div>
          </Panel>
          ...
          {/* SLA Drift Detection */}
          <Panel>
            <h3 className="text-sm font-semibold text-[#F4F7FA] mb-4" style={{ fontFamily: SORA }}>SLA Drift Detection</h3>
            <div className="grid grid-cols-3 gap-3 mb-4">
              <MetricCard label="Ticket Volume" value="+18%" sub="30-day trend" color="#F59E0B" />
              <MetricCard label="Resolution Time" value="+14%" sub="vs baseline" color="#FF6A00" alert />
              <MetricCard label="Breach Probability" value="37%" sub="Next 30 days" color="#FF6A00" alert />
            </div>
            <HeatBar label="Team Capacity Utilisation" value={87} max={100} color="#F59E0B" />
            <HeatBar label="Ticket Growth vs Headcount" value={118} max={150} color="#FF6A00" suffix="% ratio" />
          </Panel>

          {/* Contract Margin Monitor */}
          <Panel>
            <h3 className="text-sm font-semibold text-[#F4F7FA] mb-4" style={{ fontFamily: SORA }}>Contract Margin Heat Map</h3>
            {[
        ...
        <div className="space-y-4">
          ...
          <Panel>
            <h3 className="text-sm font-semibold text-[#F4F7FA] mb-3" style={{ fontFamily: SORA }}>Intelligence Pulse</h3>
            <div className="grid grid-cols-2 gap-2">
              <MetricCard label="Contracts Tracked" value="12" color="#10B981" />
              <MetricCard label="Signals (30d)" value="847" color="#3B82F6" />
              <MetricCard label="Alerts" value="6" color="#FF6A00" />
