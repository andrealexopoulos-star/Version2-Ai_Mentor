        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <Panel>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-semibold text-[#F4F7FA]" style={{ fontFamily: SORA }}>Churn Cohort Risk Snapshot</h3>
              <span className="text-xs px-2 py-0.5 rounded" style={{ fontFamily: MONO, color: '#FF6A00', background: '#FF6A0015' }}>$8.2K MRR at risk</span>
            </div>
            <div className="grid grid-cols-3 gap-3 mb-4">
              <MetricCard label="Monthly Churn" value="4.2%" sub="Target: <3%" color="#FF6A00" alert />
              <MetricCard label="At-Risk Accounts" value="6" sub="of 84 total" color="#F59E0B" />
              <MetricCard label="MRR at Risk" value="$8,200" sub="$98.4K annualised" color="#FF6A00" alert />
            </div>
            ...
            {[
              ...
              <div key={a.name} className="p-3 rounded-lg mb-2" style={{ background: '#0F1720', border: '1px solid #243140' }}>
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs font-medium text-[#F4F7FA]" style={{ fontFamily: SORA }}>{a.name}</span>
                  <span className="text-xs" style={{ fontFamily: MONO, color: a.risk === 'high' ? '#FF6A00' : '#F59E0B' }}>{a.mrr}/mo</span>
                </div>
                <p className="text-[11px] text-[#9FB0C3]" style={{ fontFamily: INTER }}>{a.signal}</p>
                <span className="text-[10px]" style={{ fontFamily: MONO, color: '#FF6A00' }}>Usage: {a.usage}% (30d)</span>
              </div>
            ))}
          </Panel>
          ...
          {/* Pipeline Velocity */}
          <Panel>
            <h3 className="text-sm font-semibold text-[#F4F7FA] mb-4" style={{ fontFamily: SORA }}>Pipeline Velocity Monitor</h3>
            <div className="grid grid-cols-4 gap-3 mb-4">
              <MetricCard label="Active Deals" value="14" color="#F4F7FA" />
              <MetricCard label="Win Rate" value="22%" sub="Target: 28%" color="#F59E0B" />
              <MetricCard label="Avg Cycle" value="38d" sub="Up from 31d" color="#FF6A00" alert />
              <MetricCard label="Pipeline Value" value="$42K" sub="MRR" color="#F4F7FA" />
            </div>
            <HeatBar label="Demo → Trial" value={45} max={100} color="#3B82F6" />
            <HeatBar label="Trial → Close" value={22} max={100} color="#FF6A00" />
            <HeatBar label="30-Day Pipeline Coverage" value={68} max={100} color="#F59E0B" />
          </Panel>

          {/* ARR & Runway */}
          <Panel>
            <h3 className="text-sm font-semibold text-[#F4F7FA] mb-4" style={{ fontFamily: SORA }}>Cash Runway & ARR Projection</h3>
            <div className="grid grid-cols-3 gap-3 mb-4">
              <MetricCard label="Current ARR" value="$1.14M" color="#F4F7FA" />
              <MetricCard label="MRR" value="$95K" color="#F4F7FA" />
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div className="p-3 rounded" style={{ background: '#FF6A0010', border: '1px solid #FF6A0020' }}>
                <span className="text-[10px] text-[#64748B] block" style={{ fontFamily: MONO }}>Worst Case</span>
                <span className="text-xs text-[#FF6A00]" style={{ fontFamily: MONO }}>5.8mo runway</span>
