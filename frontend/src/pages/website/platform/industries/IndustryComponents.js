  ...
  return (
    <div className="rounded-lg overflow-hidden" style={{ background: '#141C26', border: `1px solid ${c}20` }}>
      <button onClick={() => setOpen(!open)} className="w-full flex items-center gap-3 px-4 py-3.5 text-left">
        ...
        <span className="text-sm font-medium text-[#F4F7FA] flex-1" style={{ fontFamily: fontFamily.display }}>{title}</span>
      ...
      {open && (
        <div className="px-4 pb-4 pt-2 space-y-2" style={{ borderTop: '1px solid #243140' }}>
          ...
          <div><span className="text-[10px] text-[#64748B] uppercase" style={{ fontFamily: fontFamily.mono }}>Financial Impact</span><p className="text-sm text-[#FF6A00]" style={{ fontFamily: fontFamily.mono }}>{impact}</p></div>
          <div><span className="text-[10px] text-[#64748B] uppercase" style={{ fontFamily: fontFamily.mono }}>Intervention Window</span><p className="text-sm text-[#F4F7FA]" style={{ fontFamily: fontFamily.mono }}>{w}</p></div>
        </div>
      )}
    </div>
  );
...
export const ExecMemo = ({ memo }) => (
  <Panel>
    <h3 className="text-sm font-semibold text-[#F4F7FA] mb-3" style={{ fontFamily: fontFamily.display }}>Executive Memo</h3>
...
export const HeatBar = ({ label, value, max = 100, color = '#FF6A00', suffix = '%', alert = false }) => (
  <div className="mb-3">
    <div className="flex justify-between mb-1">
      ...
      <span className="text-xs font-semibold" style={{ fontFamily: fontFamily.mono, color: alert ? '#FF6A00' : '#F4F7FA' }}>{value}{suffix}</span>
    </div>
