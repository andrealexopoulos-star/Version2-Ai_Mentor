  ...
  <WebsiteLayout>
    ...
    <section className="relative overflow-hidden" data-testid="integrations-hero">
      ...
      <div className="max-w-5xl mx-auto px-6 pt-16 pb-14 relative z-10">
        <div className="max-w-3xl">
          <span className="text-xs font-medium tracking-widest uppercase text-[#FF6A00] mb-6 block" style={{ fontFamily: fontFamily.mono }}>Integrations</span>
    ...
    <section className="py-16" style={{ background: '#141C26' }} data-testid="integration-grid">
      <div className="max-w-5xl mx-auto px-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <span className="text-xs font-medium tracking-widest uppercase text-[#FF6A00]" style={{ fontFamily: fontFamily.mono }}>Supported Integrations</span>
        </div>
        <h2 className="text-3xl font-bold mb-12" style={{ fontFamily: fontFamily.displayING, color: '#FFFFFF' }}>Direct integrations with the platforms you already use.</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {integrationGroups.map((group) => (
            <div key={group.category} className="rounded-2xl p-6" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
              <h3 className="text-xs font-semibold tracking-widest uppercase text-[#FF6A00]/70 mb-5" style={{ fontFamily: fontFamily.mono }}>{group.category}</h3>
                ...
                {group.items.map((name) => (
                  <div key={name} className="flex items-center gap-3 px-4 py-3 rounded-xl transition-all group cursor-default hover:bg-[#FF6A00]/5" style={{ border: '1px solid rgba(255,255,255,0.06)' }}>
                    <div className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0 transition-all" style={{ background: 'rgba(159,176,195,0.08)' }}>
                      <span className="text-xs font-bold text-[#9FB0C3] group-hover:text-[#FF6A00] transition-colors" style={{ fontFamily: fontFamily.mono }}>{name.charAt(0)}</span>
                    </div>
                    <span className="text-sm font-medium text-[#F4F7FA] group-hover:text-white" style={{ fontFamily: fontFamily.displayING, color: '#FFFFFF' }}>{name}</span>
        ...
        {/* Custom APIs */}
        <div className="mt-10">
          <h3 className="text-xs font-semibold tracking-widest uppercase text-[#FF6A00]/70 mb-5" style={{ fontFamily: fontFamily.mono }}>Custom & Open APIs</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {customAPIs.map((api) => (
              <div key={api.label} className="rounded-xl p-5 transition-all hover:border-[#FF6A00]/20" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
                ...
                <h4 className="text-sm font-semibold mb-1" style={{ fontFamily: fontFamily.displayING, color: '#FFFFFF' }}>{api.label}</h4>
                <p className="text-xs text-[#9FB0C3]" style={{ fontFamily: fontFamily.body }}>{api.desc}</p>
              </div>
            ))}
          </div>
        </div>
    ...
    <section className="py-16" data-testid="how-integration-works">
      <div className="max-w-5xl mx-auto px-6">
        <div className="flex items-center gap-3 mb-4">
          <div className="w-8 h-[2px]" style={{ background: '#FF6A00' }} />
          <span className="text-xs font-medium tracking-widest uppercase text-[#FF6A00]" style={{ fontFamily: fontFamily.mono }}>How Integration Works</span>
        </div>
        <h2 className="text-3xl font-bold mb-12" style={{ fontFamily: fontFamily.displayING, color: '#FFFFFF' }}>Enterprise-grade security. Zero complexity.</h2>
    ...
    <section className="py-20" style={{ background: '#141C26' }} data-testid="data-control">
      <div className="max-w-3xl mx-auto px-6">
        <div className="rounded-2xl p-8 text-center" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,106,0,0.15)' }}>
          ...
          <h2 className="text-2xl font-bold mb-6" style={{ fontFamily: fontFamily.displayING, color: '#FFFFFF' }}>Your Data. Your Control.</h2>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            {[
              ...
              <div key={item.title}>
                <h4 className="text-sm font-semibold mb-1 text-[#FF6A00]" style={{ fontFamily: fontFamily.displayING, color: '#FFFFFF' }}>{item.title}</h4>
                <p className="text-xs text-[#9FB0C3]" style={{ fontFamily: fontFamily.body }}>{item.desc}</p>
              </div>
            ))}
          </div>
        </div>
