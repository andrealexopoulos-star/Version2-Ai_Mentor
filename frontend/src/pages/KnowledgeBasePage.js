  ...
  return (
    <div className="rounded-lg overflow-hidden" style={{ background: 'var(--biqc-bg-card)', border: '1px solid var(--biqc-border)' }}>
      <button onClick={() => setOpen(!open)} className="w-full flex items-center justify-between px-5 py-4 text-left hover:bg-white/[0.02] transition-colors" data-testid={`faq-${q.substring(0,20).replace(/\s/g,'-').toLowerCase()}`}>
        <span className="text-sm font-medium text-[var(--ink-display)] pr-4" style={{ fontFamily: fontFamily.body }}>{q}</span>
        {open ? <ChevronUp className="w-4 h-4 text-[var(--ink-muted)] shrink-0" /> : <ChevronDown className="w-4 h-4 text-[var(--ink-muted)] shrink-0" />}
      </button>
      {open && (
        <div className="px-5 pb-4" style={{ borderTop: '1px solid var(--biqc-border)' }}>
          <p className="text-sm text-[var(--ink-secondary)] leading-relaxed pt-3" style={{ fontFamily: fontFamily.body }}>{a}</p>
        </div>
      )}
    </div>
  );
    ...
    <div className="max-w-[900px] mx-auto space-y-8 px-6 py-8" style={{ fontFamily: fontFamily.body }} data-testid="knowledge-base-page">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-7 gap-2">
        {GUIDES.map(g => (
          <a key={g.id} href={`#${g.id}`} className="p-3 rounded-lg text-center transition-all hover:bg-white/[0.03]"
            ...
            <span className="text-[9px] text-[var(--ink-secondary)] block" style={{ fontFamily: fontFamily.mono }}>{g.time}</span>
          </a>
        ))}
      </div>
        ...
        <div key={guide.id} id={guide.id} className="scroll-mt-20">
          <Panel>
            <div className="flex items-start gap-4 mb-5">
              ...
              <div>
                <h2 className="text-lg font-semibold text-[var(--ink-display)] mb-1" style={{ fontFamily: fontFamily.display }}>{guide.title}</h2>
                <p className="text-sm text-[var(--ink-secondary)] leading-relaxed">{guide.why}</p>
              </div>
            </div>
              ...
              {guide.steps.map((s, i) => (
                <div key={i} className="flex items-start gap-3 overflow-hidden">
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p className="text-sm font-semibold text-[var(--ink-display)]">{s.step}</p>
                    <p className="text-xs text-[var(--ink-muted)] mt-0.5 leading-relaxed">{s.detail}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            {guide.path && (
              <div className="mt-5 pt-4" style={{ borderTop: '1px solid var(--biqc-border)' }}>
                ...
                <span className="text-[10px] text-[var(--ink-muted)] ml-3" style={{ fontFamily: fontFamily.mono }}>Estimated time: {guide.time}</span>
              </div>
            )}
          </Panel>
        </div>
      ...
      <div id="faqs" className="scroll-mt-20">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h2 className="text-xl font-semibold text-[var(--ink-display)]" style={{ fontFamily: fontFamily.display }}>Frequently Asked Questions</h2>
        </div>
      ...
      <Panel className="text-center py-8">
        <h3 className="text-lg font-semibold text-[var(--ink-display)] mb-2" style={{ fontFamily: fontFamily.display }}>Ready to get started?</h3>
        <p className="text-sm text-[var(--ink-secondary)] mb-4">Experience sovereign business intelligence in under 5 minutes.</p>
