    ...
    <div className="min-h-[calc(100vh-40px)] flex flex-col" style={{ background: 'var(--biqc-bg)' }}>
      <header className="px-6 py-3 flex items-center justify-between" style={{ borderBottom: '1px solid var(--biqc-border)' }}>
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-semibold text-sm text-[var(--ink-display)]" style={{ fontFamily: fontFamily.display }}>BIQc Setup</h3>
            <p className="text-xs text-[var(--ink-muted)]" style={{ fontFamily: fontFamily.mono }}>{cfg.title}</p>
          </div>
        </div>
        {stepNum > 0 && <div className="flex items-center gap-3">
          <span className="text-xs text-[var(--ink-muted)]" style={{ fontFamily: fontFamily.mono }}>{stepNum}/7</span>
      ...
      <div className="flex-1 flex items-center justify-center p-6">
        <div className="w-full max-w-2xl rounded-xl p-8" style={{ background: 'var(--biqc-bg-card)', border: '1px solid var(--biqc-border)' }}>
          <div className="text-center mb-6">
            ...
            <h1 className="text-2xl font-bold text-[var(--ink-display)] mb-2" style={{ fontFamily: fontFamily.display }}>{cfg.title}</h1>
            <p className="text-sm text-[var(--ink-secondary)]" style={{ fontFamily: fontFamily.body }}>{cfg.sub}</p>
          </div>
          {cfg.items && <div className="space-y-3 max-w-lg mx-auto">{cfg.items.map((item, i) => (
            <div key={i} className="flex items-start gap-3 p-3 rounded-lg" style={{ background: 'var(--biqc-bg)', border: '1px solid var(--biqc-border)' }}>
              ...
              <p className="text-sm text-[var(--ink-secondary)]">{item}</p>
            </div>
          ))}</div>}
          {cfg.fields && <div className="space-y-4 max-w-lg mx-auto">{cfg.fields.map((f, i) => (
            <div key={i}>
              <label className="text-xs font-semibold text-[var(--ink-secondary)] uppercase tracking-wider block mb-1" style={{ fontFamily: fontFamily.mono }}>{f}</label>
...
const AuthLoadingPreview = () => (
  <div className="min-h-[calc(100vh-40px)] flex items-center justify-center" style={{ background: 'var(--biqc-bg)' }}>
    <div className="text-center space-y-6">
      ...
      <div>
        <p className="text-lg font-semibold text-[var(--ink-display)]" style={{ fontFamily: fontFamily.display }}>Good evening.</p>
        <p className="text-sm text-[var(--ink-muted)] mt-1" style={{ fontFamily: fontFamily.body }}>Establishing secure connection...</p>
      </div>
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--biqc-bg)' }}>
      <div className="fixed top-0 left-0 right-0 z-50 flex items-center gap-1 px-3 py-1.5 overflow-x-auto" style={{ background: 'var(--biqc-bg-input)', borderBottom: '1px solid var(--biqc-border)' }}>
        <span className="text-[10px] text-[var(--ink-muted)] mr-1 shrink-0" style={{ fontFamily: fontFamily.mono }}>Screen:</span>
        {SCREENS.map(s => (
          <button key={s} onClick={() => setScreen(s)} className="px-2 py-1 rounded text-[10px] font-medium shrink-0"
            style={{ background: screen === s ? '#E85D00' : 'var(--surface)', color: screen === s ? 'white' : 'var(--ink-muted)', border: `1px solid ${screen === s ? '#E85D00' : 'rgba(140,170,210,0.15)'}` }}>
            {s}
          </button>
        ))}
      </div>
      ...
      <div className="pt-10" style={{ minHeight: 'calc(100vh - 40px)' }}>
          ...
          <div className="min-h-[calc(100vh-40px)] flex items-center justify-center p-4" style={{ background: 'var(--biqc-bg)' }}>
            <div className="max-w-2xl w-full rounded-xl p-8 md:p-12" style={{ background: 'var(--biqc-bg-card)', border: '1px solid var(--biqc-border)' }}>
              <div className="text-center mb-8">
                ...
                <h1 className="text-2xl md:text-3xl font-bold text-[var(--ink-display)] mb-3" style={{ fontFamily: fontFamily.display }}>Welcome to BIQc</h1>
                <p className="text-base text-[var(--ink-secondary)]" style={{ fontFamily: fontFamily.body }}>Let's set up your intelligent advisor</p>
              </div>
              <div className="p-4 rounded-lg mb-8" style={{ background: 'var(--biqc-bg)', border: '1px solid var(--biqc-border)' }}>
                <p className="text-sm text-[var(--ink-secondary)] leading-relaxed" style={{ fontFamily: fontFamily.body }}>BIQc learns about your business to provide personalized intelligence. This 5-minute setup helps us understand your context and deliver relevant insights.</p>
              </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <h3 className="text-lg font-semibold text-[var(--ink-display)]" style={{ fontFamily: fontFamily.display }}>Complete Setup Now</h3>
                      </div>
                      <p className="text-sm text-[var(--ink-secondary)]">5 minutes to unlock personalized intelligence and insights</p>
                    </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <h3 className="text-lg font-semibold text-[var(--ink-display)]" style={{ fontFamily: fontFamily.display }}>I'll Do This Later</h3>
                      </div>
                      <p className="text-sm text-[var(--ink-muted)]">You'll be asked again next time you sign in</p>
                    </div>
                    <span className="text-[var(--ink-muted)] text-xl">&#8594;</span>
                  </div>
                </button>
              </div>
              <p className="text-xs text-center mt-6 text-[var(--ink-muted)]" style={{ fontFamily: fontFamily.mono }}>You can complete this anytime from Settings</p>
            </div>
          </div>
        )}
        ...
        {screen === 'first-login-notif' && (
          <div className="min-h-[calc(100vh-40px)] relative" style={{ background: 'var(--biqc-bg)' }}>
            <div className="absolute top-4 right-4 w-[360px] rounded-xl overflow-hidden shadow-2xl" style={{ background: 'var(--biqc-bg-card)', border: '1px solid #E85D0030' }}>
              <div className="p-4">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <button className="p-1 rounded-lg text-[var(--ink-muted)]">&#10005;</button>
                </div>
                <p className="text-xs text-[var(--ink-secondary)] mb-4 leading-relaxed" style={{ fontFamily: fontFamily.body }}>Connect your email and integrations to unlock full intelligence capabilities.</p>
                <div className="space-y-2">
                  <div className="flex items-center gap-3 px-3 py-2.5 rounded-lg" style={{ background: 'var(--biqc-bg)', border: '1px solid var(--biqc-border)' }}>
                    ...
                    <div className="flex-1"><span className="text-sm text-[var(--ink-display)] block">Connect Email</span><span className="text-[10px] text-[var(--ink-muted)]" style={{ fontFamily: fontFamily.mono }}>Outlook or Gmail</span></div>
                    <span className="text-[var(--ink-muted)]">&#8594;</span>
                  </div>
                  <div className="flex items-center gap-3 px-3 py-2.5 rounded-lg" style={{ background: 'var(--biqc-bg)', border: '1px solid var(--biqc-border)' }}>
                    <span className="text-[#E85D00]">&#128268;</span>
                    <div className="flex-1"><span className="text-sm text-[var(--ink-display)] block">Connect Integrations</span><span className="text-[10px] text-[var(--ink-muted)]" style={{ fontFamily: fontFamily.mono }}>Xero, HubSpot, CRM</span></div>
                    <span className="text-[var(--ink-muted)]">&#8594;</span>
                  </div>
                </div>
              </div>
            ...
            <div className="flex items-center justify-center h-full pt-20">
              <p className="text-sm text-[var(--ink-muted)]" style={{ fontFamily: fontFamily.mono }}>First-login notification (appears top-right on dashboard)</p>
            </div>
          </div>
        )}
        ...
        {screen === 'checkin-alert' && (
          <div className="min-h-[calc(100vh-40px)] p-8" style={{ background: 'var(--biqc-bg)' }}>
            <h2 className="text-lg font-semibold text-[var(--ink-display)] mb-4" style={{ fontFamily: fontFamily.display }}>Weekly Check-In Alert (appears on dashboard)</h2>
            <div className="max-w-3xl space-y-3">
              <div className="rounded-2xl p-4 flex items-start gap-3" style={{ background: '#7C3AED10', border: '1px solid #7C3AED25' }}>
                ...
                <div className="flex-1">
                  <p className="text-sm font-semibold text-[var(--ink-display)]" style={{ fontFamily: fontFamily.display }}>Weekly Check-In Available</p>
                  <p className="text-xs mt-0.5 text-[var(--ink-secondary)]" style={{ fontFamily: fontFamily.body }}>Schedule a video check-in with your BIQc advisor to review progress and priorities.</p>
                  <div className="flex gap-2 mt-3">
                    ...
                    <button className="text-[11px] px-2 py-1.5 rounded-lg text-[var(--ink-muted)]">&#10005;</button>
                  </div>
                </div>
              </div>
              <div className="rounded-2xl p-4 flex items-start gap-3" style={{ background: '#E85D0010', border: '1px solid #E85D0025' }}>
                ...
                <div className="flex-1">
                  <p className="text-sm font-semibold text-[var(--ink-display)]" style={{ fontFamily: fontFamily.display }}>Recalibration Recommended</p>
                  <p className="text-xs mt-0.5 text-[var(--ink-secondary)]" style={{ fontFamily: fontFamily.body }}>Your business context may have changed. A quick recalibration keeps BIQc accurate.</p>
                  <div className="flex gap-2 mt-3">
