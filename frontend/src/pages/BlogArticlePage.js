...
const RenderContent = ({ content }) => {
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith('## ')) {
      elements.push(<h2 key={i} className="text-xl font-semibold mt-8 mb-3" style={{ color: '#F4F7FA', fontFamily: HEAD }}>{line.replace('## ', '')}</h2>);
      ...
      elements.push(
        <div key={i} className="my-3 p-3 rounded-lg flex items-start gap-2" style={{ background: '#0F1720', border: '1px solid #243140' }}>
          ...
          <div>
            <span className="text-[10px] font-semibold tracking-wider uppercase block mb-0.5" style={{ color: '#FF6A00', fontFamily: MONO }}>Source</span>
            <span className="text-xs text-[#9FB0C3]" style={{ fontFamily: BODY }}>{sourceText}</span>
            {urlMatch && (
              <a href={urlMatch[0]} target="_blank" rel="noopener noreferrer" className="text-[10px] text-[#3B82F6] hover:text-[#60A5FA] block mt-0.5 break-all" style={{ fontFamily: MONO }}>
                {urlMatch[0]}
              </a>
            )}
          </div>
      ...
      elements.push(
        <p key={i} className="text-sm leading-relaxed mb-4" style={{ color: '#9FB0C3', fontFamily: BODY }}>
          {parts.map((part, j) => {
    ...
    <WebsiteLayout>
      <div style={{ background: '#0F1720', minHeight: '100vh', paddingTop: '64px' }} data-testid="blog-article-page">
        <article className="max-w-3xl mx-auto px-6 py-12">
          {/* Back */}
          <Link to="/blog" className="inline-flex items-center gap-2 mb-8 text-sm text-[#64748B] hover:text-[#FF6A00] transition-colors" style={{ fontFamily: BODY }} data-testid="blog-back">
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div className="flex items-center gap-1 text-[10px] text-[#64748B]" style={{ fontFamily: MONO }}>
          ...
          {/* Title */}
          <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold mb-4 leading-tight" style={{ color: '#F4F7FA', fontFamily: HEAD }}>{article.title}</h1>
          <p className="text-base text-[#9FB0C3] mb-8 leading-relaxed" style={{ fontFamily: BODY }}>{article.excerpt}</p>
          ...
          <div className="rounded-xl p-6 text-center mb-8" style={{ background: '#141C26', border: '1px solid #243140' }}>
            <h3 className="text-lg font-semibold mb-2" style={{ color: '#F4F7FA', fontFamily: HEAD }}>See these insights in action for your business</h3>
            <p className="text-sm text-[#9FB0C3] mb-4" style={{ fontFamily: BODY }}>BIQc surfaces real intelligence from your connected business systems.</p>
          ...
          {relatedArticles.length > 0 && (
            <div>
              <h3 className="text-lg font-semibold mb-4" style={{ color: '#F4F7FA', fontFamily: HEAD }}>Related Articles</h3>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {relatedArticles.map(ra => (
                  <Link key={ra.slug} to={`/blog/${ra.slug}`}
                    ...
                    <span className="text-[10px] px-2 py-0.5 rounded mb-2 inline-block" style={{ color: CAT_COLORS[ra.category] || '#FF6A00', background: (CAT_COLORS[ra.category] || '#FF6A00') + '15', fontFamily: MONO }}>{ra.category}</span>
                    <h4 className="text-sm font-semibold mb-1 line-clamp-2" style={{ color: '#F4F7FA', fontFamily: HEAD }}>{ra.title}</h4>
                    <span className="text-[10px] text-[#64748B]" style={{ fontFamily: MONO }}>{ra.readTime}</span>
                  </Link>
                ))}
              </div>
            </div>
