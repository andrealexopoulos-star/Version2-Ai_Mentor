...
const SubscribePage = () => {
  ...
  const showTrialStatus = !isSubscribedUser && ['free', 'trial'].includes(currentTier);
  const currentPlanLabel = ['free', 'trial'].includes(currentTier) ? 'Trial / not yet activated' : currentTier;

  return (
    <div className="min-h-screen flex flex-col items-center px-4 py-8 md:px-6 md:py-10 lg:py-12 pb-28 md:pb-16 max-w-7xl mx-auto w-full" style={{ background: 'var(--canvas-app, var(--surface))' }} data-testid="subscribe-page">
      ...
      <div className="text-center mb-8 md:mb-10 max-w-2xl">
        ...
        {(isSubscribedUser || showTrialStatus) && (
          <p className="text-sm" style={{ color: 'var(--ink-secondary)', fontFamily: 'var(--font-ui)' }}>
            Current plan: <strong style={{ color: 'var(--ink-display)', textTransform: ['free', 'trial'].includes(currentTier) ? 'none' : 'capitalize' }}>{currentPlanLabel}</strong>
          </p>
        )}
      ...
      <div className="w-full max-w-5xl mb-5 md:mb-6 p-3.5 md:p-4" style={{ borderRadius: 'var(--r-lg)', border: '1px solid var(--border)', background: 'var(--surface)', boxShadow: 'var(--elev-1)' }}>
      ...
      <div className="w-full max-w-5xl mb-6 p-3.5 md:p-4" style={{ borderRadius: 'var(--r-lg)', border: '1px solid var(--border)', background: 'var(--surface)', boxShadow: 'var(--elev-1)' }}>
      ...
      <div className="grid grid-cols-1 gap-4 md:gap-5 xl:gap-6 w-full max-w-7xl mb-6 md:mb-8 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4" style={{ alignItems: 'stretch' }}>
        {PLANS.map(plan => {
          ...
          return (
            ...
            >
              {isCurrent && (
                <span className="absolute top-3 right-3 text-[10px] font-semibold px-3 py-1 rounded-full whitespace-nowrap" style={{ background: 'var(--lava-wash)', color: 'var(--lava-deep, var(--lava))', fontFamily: 'var(--font-mono)', letterSpacing: 'var(--ls-caps, 0.08em)', textTransform: 'uppercase' }}>Current plan</span>
              )}
              {isPopular && !isCurrent && (
                <span className="absolute top-3 right-3 text-[10px] font-semibold px-3 py-1 rounded-full text-white whitespace-nowrap" style={{ background: 'var(--ink-display, #0A0A0A)', fontFamily: 'var(--font-mono)', letterSpacing: 'var(--ls-caps, 0.08em)', textTransform: 'uppercase' }}>Most popular</span>
              )}
              <h3 className="text-[24px] md:text-[26px] font-semibold mb-1 pr-20" style={{ fontFamily: 'var(--font-display)', color: 'var(--ink-display)', lineHeight: 1.05 }}>{plan.name}</h3>
              <p className="text-sm mb-5 min-h-[42px]" style={{ color: 'var(--ink-secondary)', fontFamily: 'var(--font-ui)', lineHeight: 1.45 }}>{plan.tagline || ''}</p>
              <div className="flex items-end gap-2 mb-1 flex-wrap">
                <span style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(2.25rem, 4vw, 2.9rem)', lineHeight: 1, letterSpacing: '-0.03em', color: 'var(--ink-display)', wordBreak: 'break-word' }}>{plan.price}</span>
                <span className="text-sm" style={{ color: 'var(--ink-muted)', fontFamily: 'var(--font-mono)' }}>{plan.period}</span>
              </div>
              <div className="mb-5" style={{ minHeight: 18 }} />
              {isCurrent ? (
                <div className="w-full py-3.5 text-center text-sm font-semibold" style={{ borderRadius: 'var(--r-lg)', border: '1px solid var(--border-strong, var(--border))', color: 'var(--ink-secondary)', fontFamily: 'var(--font-ui)', marginBottom: 'var(--sp-5, 20px)', cursor: 'default' }}>Current Plan</div>
              ) : (
      ...
      <div className="w-full max-w-7xl mb-6 md:mb-8 p-4 md:p-5" style={{ borderRadius: 'var(--r-lg)', border: '1px solid var(--border)', background: 'var(--surface)', boxShadow: 'var(--elev-1)' }}>
        <p className="text-[10px] uppercase tracking-[0.14em]" style={{ color: 'var(--lava)', fontFamily: 'var(--font-mono)', letterSpacing: 'var(--ls-caps, 0.08em)' }}>
          Not sure which plan fits?
        </p>
        <div className="mt-2 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <p className="text-sm max-w-3xl" style={{ color: 'var(--ink-secondary)', fontFamily: 'var(--font-ui)' }}>
            Get quick rollout guidance before checkout and choose the right plan with confidence.
          </p>
          <button
            type="button"
            onClick={() => navigate('/speak-with-local-specialist')}
            className="w-full md:w-auto px-4 py-2.5 text-sm font-semibold text-white"
      ...
      <div className="w-full max-w-5xl mb-8 p-5" style={{ borderRadius: 'var(--r-lg)', border: '1px solid var(--border)', background: 'var(--surface)', boxShadow: 'var(--elev-1)' }}>
      ...
      {(section === 'foundation' || section === 'advanced') && (
        <div className="w-full max-w-5xl mb-6 p-3" style={{ borderRadius: 'var(--r-lg)', background: 'var(--lava-wash)', border: '1px solid var(--lava)' }}>
          <p className="text-xs" style={{ color: 'var(--warning)', fontFamily: 'var(--font-ui)' }}>
            You were redirected from a legacy subscription path. The related module references are shown below.
          </p>
        </div>
      )}
      ...
      <div className="w-full max-w-5xl mt-4 mb-8" style={{ borderRadius: 'var(--r-lg)', border: '1px solid var(--border)', background: 'var(--surface)', boxShadow: 'var(--elev-1)' }}>
        <button
          type="button"
          onClick={() => setModulesExpanded((prev) => !prev)}
          className="w-full px-4 py-3 flex items-center justify-between text-left"
          style={{ fontFamily: 'var(--font-ui)', color: 'var(--ink-display)' }}
          data-testid="subscribe-view-more-modules"
        >
          <span className="text-sm font-semibold">View more platform modules</span>
          <ChevronDown className="w-4 h-4 transition-transform duration-200" style={{ color: 'var(--ink-muted)', transform: modulesExpanded ? 'rotate(180deg)' : 'rotate(0deg)' }} />
        </button>
        {modulesExpanded && (
          <div className="px-4 pb-4">
            <div
              ...
              style={{ borderRadius: 'var(--r-lg)', border: '1px solid var(--border)', background: 'var(--surface)' }}
                ...
                {FOUNDATION_FEATURES.map((feature) => (
                  <div key={feature.key} className="p-3" style={{ borderRadius: 'var(--r-md, 8px)', border: '1px solid var(--border)' }}>
                    ...
                    <div className="mt-2 flex items-center justify-between">
                      ...
                      >
                        {foundationUnlocked ? 'Included in your tier' : 'Available with paid tier'}
                      </span>
                      <button
                        type="button"
            ...
            >
              ...
              <p className="text-xs mt-2" style={{ color: 'var(--ink-secondary)', fontFamily: 'var(--font-ui)' }}>
                These modules are staged for rollout. Join the waitlist for priority access and packaging feedback.
              </p>
      ...
      <div className="w-full max-w-5xl mt-14 text-center relative overflow-hidden" style={{ padding: 'var(--sp-12, 48px) var(--sp-6, 24px)', background: 'linear-gradient(160deg, #F8FAFC, #F0F4F8)', borderRadius: 'var(--r-2xl)' }}>
        {/* Lava radial glow */}
