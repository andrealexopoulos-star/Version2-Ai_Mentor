      ...
      <div className="space-y-4 max-w-[1200px]" style={{ fontFamily: fontFamily.body }} data-testid="support-console">
        ...
        <div className="rounded-xl p-4 flex items-center gap-3" style={{ background: '#10B98108', border: '1px solid #10B98125' }}>
          ...
          <div>
            ...
            <span className="text-xs text-[var(--ink-muted)] ml-3" style={{ fontFamily: fontFamily.mono }}>{admin.email} | {admin.user_id?.substring(0, 8)}</span>
          </div>
        </div>

        {/* Feature Flags */}
        <div className="rounded-xl p-4" style={{ background: 'var(--biqc-bg-card)', border: '1px solid var(--biqc-border)' }}>
          <span className="text-[10px] text-[var(--ink-muted)] block mb-2" style={{ fontFamily: fontFamily.mono }}>Feature Flags</span>
        ...
        {[{ id: 'users', label: 'Users & Access', icon: Users }, { id: 'contacts', label: 'Enterprise Leads', icon: Eye }, { id: 'audit', label: 'Audit Log', icon: AlertTriangle }].map(t => (
            <button key={t.id} onClick={() => { setTab(t.id); if (t.id === 'audit') loadAudit(); if (t.id === 'contacts') loadContacts(); }}
              className={`flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-md text-xs font-medium ${tab === t.id ? 'text-[var(--ink-display)]' : 'text-[var(--ink-muted)]'}`}
        ...
        {tab === 'users' && (
          <div className="rounded-xl overflow-hidden" style={{ background: 'var(--biqc-bg-card)', border: '1px solid var(--biqc-border)' }}>
                ...
                <thead>
                  <tr style={{ borderBottom: '1px solid var(--biqc-border)' }}>
                    {['Email', 'Name', 'Business', 'Tier', 'Status', 'Actions'].map(h => (
                      <th key={h} className="px-3 py-2 text-left text-[var(--ink-muted)]" style={{ fontFamily: fontFamily.mono }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {users.map(u => {
                    ...
                    return (
                      <tr key={u.id} style={{ borderBottom: '1px solid var(--biqc-border)' }} className={u.is_disabled ? 'opacity-50' : ''}>
                        <td className="px-3 py-2 text-[var(--ink-display)]">{u.email}</td>
            ...
            <div className="px-3 py-2 text-[10px] text-[var(--ink-muted)]" style={{ fontFamily: fontFamily.mono }}>{users.length} users total</div>
          </div>
        )}
        ...
        {tab === 'contacts' && (
          <div className="rounded-xl overflow-hidden" style={{ background: 'var(--biqc-bg-card)', border: '1px solid var(--biqc-border)' }}>
            <div className="px-4 py-3" style={{ borderBottom: '1px solid var(--biqc-border)' }}>
              <span className="text-xs font-semibold text-[var(--ink-display)]" style={{ fontFamily: fontFamily.mono }}>Enterprise Contact Requests ({contacts.length})</span>
              <p className="text-[10px] text-[var(--ink-muted)] mt-0.5">Users requesting access to paid or waitlist modules. Route to sales follow-up when configured.</p>
            </div>
            {contacts.length === 0 && <p className="text-xs text-[var(--ink-muted)] p-4">No contact requests yet.</p>}
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                <thead>
                  <tr style={{ borderBottom: '1px solid var(--biqc-border)' }}>
                ...
                <tbody>
                  {contacts.map(c => (
                    <tr key={c.id} style={{ borderBottom: '1px solid var(--biqc-border)' }}>
                      <td className="px-3 py-2 text-[var(--ink-display)]">{c.name}</td>
                      <td className="px-3 py-2 text-[var(--ink-secondary)]">{c.business_name || '—'}</td>
                      <td className="px-3 py-2 text-[var(--ink-secondary)]">{c.email}</td>
                      <td className="px-3 py-2 text-[var(--ink-secondary)]">{c.phone || '—'}</td>
                      ...
                      <td className="px-3 py-2 text-[var(--ink-secondary)]" style={{ fontFamily: fontFamily.mono, fontSize: '10px' }}>{c.callback_date} {c.callback_time}</td>
                      <td className="px-3 py-2 text-[var(--ink-muted)] max-w-[200px]"><span className="line-clamp-2">{c.description}</span></td>
                      <td className="px-3 py-2 text-[var(--ink-muted)]" style={{ fontFamily: fontFamily.mono, fontSize: '10px' }}>{c.created_at ? new Date(c.created_at).toLocaleDateString('en-AU') : ''}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {tab === 'audit' && (
          <div className="rounded-xl p-4 space-y-1" style={{ background: 'var(--biqc-bg-card)', border: '1px solid var(--biqc-border)' }}>
            {auditLog.length === 0 && <p className="text-xs text-[var(--ink-muted)]">No admin actions recorded yet.</p>}
            {auditLog.map(a => (
              <div key={a.id} className="flex items-center gap-2 py-2 text-xs" style={{ borderBottom: '1px solid var(--biqc-border)' }}>
                ...
                <span className="text-[var(--ink-display)] flex-1">{a.action_type}</span>
                <span className="text-[var(--ink-muted)]" style={{ fontFamily: fontFamily.mono }}>{a.target_user_id?.substring(0, 8)}</span>
                <span className="text-[10px] text-[var(--ink-muted)]">{a.created_at ? new Date(a.created_at).toLocaleString('en-AU') : ''}</span>
              </div>
            ))}
          </div>
        )}
