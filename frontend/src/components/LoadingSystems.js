  ...
  return (
    <div className={`${h} flex flex-col items-center justify-center relative overflow-hidden`} data-testid="cognitive-mesh">
      <style>{`
        ...
        @keyframes meshGlow{0%,100%{box-shadow:0 0 8px rgba(255,106,0,0.1)}50%{box-shadow:0 0 20px rgba(255,106,0,0.2)}}
      `}</style>
  ...
  return (
    <div className={`${h} flex flex-col items-center justify-center relative overflow-hidden`} data-testid="radar-sweep">
      <style>{`
        ...
        @keyframes signalLock{0%{box-shadow:0 0 4px rgba(255,106,0,0.3)}50%{box-shadow:0 0 16px rgba(255,106,0,0.6)}100%{box-shadow:0 0 4px rgba(255,106,0,0.3)}}
      `}</style>
        ...
        <div className="absolute inset-0 pointer-events-none flex items-center justify-center">
          <div className="relative" style={{ width: 200, height: 200 }}>
            ...
            <svg className="absolute inset-0 w-full h-full" viewBox="0 0 200 200">
              <circle cx="100" cy="100" r="80" fill="none" stroke="#243140" strokeWidth="0.5" />
              <circle cx="100" cy="100" r="55" fill="none" stroke="#243140" strokeWidth="0.3" />
              <circle cx="100" cy="100" r="30" fill="none" stroke="#243140" strokeWidth="0.3" />
              <line x1="100" y1="20" x2="100" y2="180" stroke="#243140" strokeWidth="0.3" />
              <line x1="20" y1="100" x2="180" y2="100" stroke="#243140" strokeWidth="0.3" />
            </svg>
            {/* Sweep arc */}
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="w-32 h-32 rounded-full" style={{ border: '1px solid #FF6A0020', animation: 'radarPulse 4s ease-out infinite' }} />
            </div>
            {/* Blips */}
            {blips.map(b => (
              <div key={b.id} className="absolute w-2 h-2 rounded-full" style={{
                left: `${b.x}%`, top: `${b.y}%`, background: '#FF6A00',
                animation: `blipIn 2s ease-out forwards${b.bright ? ', signalLock 1s ease-in-out' : ''}`,
                boxShadow: b.bright ? '0 0 8px rgba(255,106,0,0.5)' : 'none',
              }} />
            ))}
            {/* Center dot */}
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-3 h-3 rounded-full" style={{ background: '#FF6A00', boxShadow: '0 0 12px rgba(255,106,0,0.4)' }} />
          </div>
        </div>
      )}
