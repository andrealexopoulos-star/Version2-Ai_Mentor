...
const PlatformLayout = ({ children, title }) => {
            ...
            <div key={group.label} className="mb-3">
              ...
              {expandedGroups[group.label] && group.items.map((item) => {
                ...
                return (
                  ...
                  >
                    <item.icon className="w-4 h-4" style={{ color: isActive ? '#FF6A00' : '#64748B' }} />
                    {item.label}
                  </Link>
                );
              })}
  ...
  return (
    <div className="min-h-screen flex" style={{ background: '#0F1720', color: '#F4F7FA' }}>
      <Sidebar />
      {mobileSidebar && <Sidebar mobile />}

      <div className="flex-1 flex flex-col min-w-0">
        {/* Top bar */}
        <header className="h-16 flex items-center justify-between px-6 shrink-0" style={{ background: '#0A1018', borderBottom: '1px solid #243140' }} data-testid="platform-topbar">
          ...
          <div className="flex items-center gap-4">
            <button className="relative p-2 rounded-md hover:bg-white/5 text-[#9FB0C3]" data-testid="notifications-btn">
              ...
              <span className="absolute top-1.5 right-1.5 w-2 h-2 rounded-full" style={{ background: '#FF6A00' }} />
            </button>
