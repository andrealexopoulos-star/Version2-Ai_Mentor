...
### Sprint 33 — Launch Packaging Redesign Correction (In Progress — Mar 2026)
- **BIQc Foundation corrected** — the $349 paid tier is now named `BIQc Foundation` across pricing, upgrade, subscription, sidebar, integrations, and Stripe checkout product naming. Scope now explicitly includes Exposure Scan, Marketing Auto, Reports, Decision Tracker, SOP Generator, Ingestion Audit, Revenue, Operations, Marketing Intelligence, and Boardroom.
- **Navigation IA corrected** — `BIQc Foundation` is now a direct page entry (`/biqc-foundation`) with detailed feature explanation and optional paid submenu only when the user has access; `BIQc Legal` is now a fixed bottom sidebar link and not collapsible.
- **More Features corrected** — paid modules were removed from waitlist inventory; remaining removed modules now route into richer `More Features` detail states with back navigation and waitlist/contact flow. Andre retains direct access to removed modules with no waitlist redirect.
- **BIQc Legal expanded** — the legal experience now includes detailed clickable tabs for Overview, AI Learning Guarantee, Security, Trust Centre, DPA, Privacy, Terms, and Enterprise Terms, with richer Australian jurisdiction and trust content.
- **Advisor Business Brain state softened** — the previous large alarming red unavailability state was redesigned into a calmer amber delayed-mode state to reduce false alarm behaviour and cognitive overload while preserving truthfulness.
- **Stripe deployment issue fixed** — removed the accidental `emergentintegrations` Stripe dependency that caused runtime import failures in deployment logs, restored plain Stripe SDK usage, and kept the updated BIQc Foundation product description in checkout creation.
- **Verification completed** — redesign validated via self-smoke checks plus testing agent iteration `154`; sidebar IA, BIQc Foundation page, More Features exclusions, BIQc Legal tabs, Andre direct access, and Advisor degraded-state redesign all verified.

### Sprint 32 — Forensic Truth Gate + Honest Advisor Recovery (In Progress — Mar 2026)
- **Forensic Truth Gate added to Business Brain** — `backend/business_brain_engine.py` now blocks or downgrades concerns when the required source is stale, unverified, proxy-only, or missing auditable sample support. BIQc no longer asserts false zero-value claims like `average cycle of 0 days`, `operating expense pressure is 0% while revenue growth is $0`, or `lead response time is 0.0 hours` when the evidence is not forensic-grade.
- **Connector truth states made explicit** — `backend/intelligence_live_truth.py` now derives connector truth states (`live`, `stale`, `unverified`, `error`) from canonical `business_core.source_runs` history and exposes `crm_state`, `accounting_state`, and `email_state` in canonical truth outputs.
- **Business Brain integrity contract expanded** — `backend/routes/business_brain.py` now returns `integrity_alerts` + `truth_summary` so the frontend can explain why certain concerns were withheld instead of silently fabricating confidence.
- **Advisor fallback path hardened** — `frontend/src/pages/AdvisorWatchtower.js` now falls back to verified watchtower / inbox / live integration signals when Business Brain concerns are blocked, and conflict detection only runs on verified signals. Result: the page stays useful without surfacing false conflict cards.
- **Advisor loading trap reduced further** — `backend/routes/email.py` no longer performs the expensive exact Outlook email-count query during `/api/outlook/status`, which brought the endpoint down to ~1.4s in self-test and removed a key cause of Advisor loading delay.
- **Verification completed** — self-tests, regression pytest (`backend/tests/test_iteration150_forensic_truth_gate.py`), testing agent iteration `151`, backend specialist checks, and frontend specialist checks all passed. Advisor now loads, shows honest degraded-state messaging, and no longer repeats the false zero-value conflict language from the user’s screenshot.
...
- **Production status note** — External production `https://biqc.ai` still showed `fallback_core_metrics` with 20 metrics at the time of verification, so the code fix now needs the next Azure deploy to make production match preview

### Sprint 12 — Production Forensic Recovery + Panel UX Move (Complete — Mar 2026)
- **Production recovery verified** — Post-deploy production retest (`iteration_133`) confirms previously broken modules are now working live on `https://biqc.ai`: War Room Q&A, Ops Advisory, Documents, and Priority Inbox
