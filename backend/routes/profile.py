...
async def calculate_business_score(profile: dict, onboarding: dict = None, user_id: str = None) -> int:
    ...
    if user_id:
        ...
        doc_count = await count_user_data_files_supabase(get_sb(), user_id)
        score += min(5, doc_count * 1)  # 1 point per doc, max 5
        
        # AI advisor conversations (5 points)
        chat_result = get_sb().table("chat_history").select("id", count="exact").eq("user_id", user_id).execute()
        ...
        sop_count = await count_sops_supabase(get_sb(), user_id)
