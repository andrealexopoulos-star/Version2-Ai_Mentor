    ...
    *,
    ...
    async with _httpx.AsyncClient(timeout=90) as client:
        async with client.stream(
            ...
            json={"contents": [{"parts": [{"text": prompt}]}], "generationConfig": {"maxOutputTokens": 2000, "temperature": 0.7}},
        ) as response:
    ...
    *,
    ...
    async with _httpx.AsyncClient(timeout=90) as client:
        async with client.stream(
            ...
            json={"model": model, "max_tokens": 2000, "temperature": 0.45, "system": system_message, "messages": [{"role": "user", "content": user_content}], "stream": True},
        ) as response:
            async for line in response.aiter_lines():
                if line.startswith("data: "):
                    try:
