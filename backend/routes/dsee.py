...
async def build_asymmetries(
    ...
    """Adaptive multi-tier asymmetry engine. Structure-aware. ≥3 guaranteed or explicit insufficient."""
    asymmetries = []
    ...
    for cr in comp_reviews:
        if cr['reviews'] > 0 and cr['reviews'] > subject_reviews['reviews'] * 1.5 and cr['reviews'] >= 10:
            ...
            asymmetries.append({'category': 'ReviewDensityGap', 'tier': 'major', 'subject_metric': f\'{subject_reviews["reviews"]} Google reviews\', 'competitor_metric': f\'{cr["name"]}: {cr["reviews"]} Google reviews\', 'metric_source': 'Google Maps', 'differential_ratio': f\'{ratio}x\', 'structural_implication': 'Trust Density Imbalance', 'confidence': min(0.66, 0.5 + (ratio / 50))})
            break
    ...
    if subject_search_position is None:
        ...
        for i, item in enumerate(search_results.get('organic', [])[:5]):
            ...
            if top_comp: break
        if top_comp:
            asymmetries.append({'category': 'SearchVisibilityGap', 'tier': 'major', 'subject_metric': f\'Not found in top 10 for "{search_query}"\', 'competitor_metric': f\'{top_comp["name"]}: Position {top_comp["position"]}\', 'metric_source': 'Google Search', 'differential_ratio': 'absent vs present', 'structural_implication': 'Visibility Compression', 'confidence': 0.60})

    for ca in comp_authorities:
        if ca['total'] > subject_authority['total'] and ca['total'] >= 2:
            asymmetries.append({'category': 'AuthorityGap', 'tier': 'major', 'subject_metric': f\'{subject_authority["total"]} authority markers\', 'competitor_metric': f\'{ca["name"]}: {ca["total"]} markers\', 'metric_source': 'Google Search', 'differential_ratio': f\'{ca["total"]}:{subject_authority["total"]}\', 'structural_implication': 'Authority Deficit', 'confidence': 0.55})
            break
    ...
    for platform, q in [('ProductReview', f'{subject_name} site:productreview.com.au'), ('Facebook', f'{subject_name} {location} facebook reviews')]:
        results = await _serper(q, num=3)
        if any(subject_name.lower() in r.get('snippet', '').lower() for r in results.get('organic', [])): platforms_found.append(platform)
    absent = [p for p in platforms_checked if p not in platforms_found]
    if len(absent) >= 2:
        asymmetries.append({'category': 'ReviewSurfaceGap', 'tier': 'major', 'subject_metric': f\'Absent from: {", ".join(absent[:3])}\', 'competitor_metric': 'Competitors present on multiple platforms', 'metric_source': 'Platform search', 'differential_ratio': f\'{len(absent)}/{len(platforms_checked)} absent\', 'structural_implication': 'Reputation Surface Fragility', 'confidence': 0.50})

    # ═══ TIER 2: MODERATE (fallback if <3) ═══
    if len(asymmetries) < 3:
        suburb_count = len(subject_structure.get('suburbs_detected', []))
        if suburb_count < 5:
            asymmetries.append({'category': 'GeographicCoverageGap', 'tier': 'moderate', 'subject_metric': f\'{suburb_count} geographic mentions\', 'competitor_metric': 'Typical local coverage 5+ suburbs', 'metric_source': 'Website analysis', 'differential_ratio': f\'{suburb_count}/5\', 'structural_implication': 'Geographic Visibility Compression', 'confidence': 0.48})
        if subject_reviews.get('rating'):
            for cr in comp_reviews:
                if cr.get('rating') and cr['rating'] > subject_reviews['rating'] and cr['reviews'] >= 10:
                    asymmetries.append({'category': 'RatingQualityGap', 'tier': 'moderate', 'subject_metric': f\'{subject_reviews["rating"]}/5 ({subject_reviews["reviews"]} reviews)\', 'competitor_metric': f\'{cr["name"]}: {cr["rating"]}/5 ({cr["reviews"]} reviews)\', 'metric_source': 'Google Maps', 'differential_ratio': f\'{cr["rating"]} vs {subject_reviews["rating"]}\', 'structural_implication': 'Trust Quality Asymmetry', 'confidence': 0.52})
                    break
        services_count = len(subject_structure.get('services', []))
        if services_count < 3:
            asymmetries.append({'category': 'ContentFrequencyGap', 'tier': 'moderate', 'subject_metric': f\'{services_count} service categories detected\', 'competitor_metric': 'Competitors typically list 4+ services', 'metric_source': 'Website analysis', 'differential_ratio': f\'{services_count}/4\', 'structural_implication': 'Service Validation Fragmentation', 'confidence': 0.45})
    ...
    if len(asymmetries) < 3 or is_large_brand:
        ...
        if len(services) >= 3 and subject_authority['total'] < 3:
            asymmetries.append({'category': 'ServiceValidationTension', 'tier': 'structural', 'subject_metric': f\'{len(services)} services listed, {subject_authority["total"]} authority markers\', 'competitor_metric': 'Breadth exceeds validation depth', 'metric_source': 'Website + Search', 'differential_ratio': f\'{len(services)}:{subject_authority["total"]}\', 'structural_implication': 'Breadth vs Depth Imbalance', 'confidence': 0.50})
        if subject_authority['awards'] > 0 and subject_authority['media'] == 0:
            asymmetries.append({'category': 'AuthorityCitationTension', 'tier': 'structural', 'subject_metric': f\'{subject_authority["awards"]} awards, 0 media\', 'competitor_metric': 'One-dimensional authority surface', 'metric_source': 'Google Search', 'differential_ratio': f\'{subject_authority["awards"]}:0\', 'structural_implication': 'Authority Dispersion Imbalance', 'confidence': 0.45})
        if is_large_brand and subject_reviews.get('reviews', 0) < 20 and location:
            asymmetries.append({'category': 'CorporateLocalMismatch', 'tier': 'structural', 'subject_metric': f\'National brand, {subject_reviews.get("reviews", 0)} local reviews in {location}\', 'competitor_metric': 'National scope vs weak local surface', 'metric_source': 'Google Maps + Website', 'differential_ratio': 'national vs local gap', 'structural_implication': 'Corporate vs Local Authority Mismatch', 'confidence': 0.48})
            ...
            if len(cities) >= 2:
                ...
                if loc_reviews and max(loc_reviews) > 0:
                    ...
                    if variance > max(loc_reviews) * 0.5:
                        asymmetries.append({'category': 'LocationTrustVariance', 'tier': 'structural', 'subject_metric': f\'Review range: {min(loc_reviews)}–{max(loc_reviews)}\', 'competitor_metric': f\'Variance: {variance} reviews across locations\', 'metric_source': 'Google Maps', 'differential_ratio': f\'{max(loc_reviews)}:{min(loc_reviews)}\', 'structural_implication': 'Franchise Trust Distribution Imbalance', 'confidence': 0.55})

    required = ['category', 'subject_metric', 'competitor_metric', 'metric_source', 'differential_ratio', 'structural_implication', 'confidence']
...
def compute_confidence(
    ...
    authority_total: int,
    asymmetry_count: int,
    domain_integrity: bool,
    public_mode: bool,
) -> Dict:
    # Source diversity: how many distinct data sources returned data
    sources = sum([
        1 if review_data.get('reviews', 0) > 0 else 0,
        1 if search_position is not None else 0,
        1 if authority_total > 0 else 0,
        1,  # DOM always present
    ])
    source_diversity = sources / 4.0

    # Cross-source corroboration: agreement between signals
    corroboration = 0.5  # base
    if review_data.get('reviews', 0) > 0 and authority_total > 0:
        corroboration += 0.2
    if search_position and search_position <= 5:
        corroboration += 0.15

    # Structure clarity
    structure_clarity = min(structure_confidence, 1.0)

    # Review layer completeness
    review_completeness = 0.3  # base (we always check Google Maps)
    if review_data.get('reviews', 0) > 0:
        review_completeness += 0.4
    if review_data.get('rating'):
        review_completeness += 0.3

    # Recency (static for now — all scans are current)
    recency = 0.8
    ...
    return {
        ...
        'cap_applied': raw > 0.70 and public_mode,
        'components': {
            'source_diversity': round(source_diversity, 3),
            'cross_source_corroboration': round(corroboration, 3),
            'structure_clarity': round(structure_clarity, 3),
            'review_layer_completeness': round(review_completeness, 3),
            'recency': round(recency, 3),
            'domain_integrity': round(domain_score, 3),
        },
    }


# ═══════════════════════════════════════════════════════════════
# AGENT 6: PROJECTION LOCK CONTROLLER
# ═══════════════════════════════════════════════════════════════

BLOCKED_PATTERNS = [
    r'\$\d', r'revenue', r'profit', r'turnover', r'income',
    r'\d+%\s*growth', r'ROI', r'return on', r'margin\s*\d',
    r'forecast', r'project(?:ed|ion)', r'financial',
    r'earning', r'valuation', r'market cap',
]

def validate_no_projections(output: Dict) -> Dict:
    """Scan all string values in output for financial projection language."""
    violations = []
