...
async def build_asymmetries(
    ...
    # === ASYMMETRY 1: Review Density Gap ===
        ...
        if cr['reviews'] > 0 and cr['reviews'] > subject_reviews['reviews'] * 1.5 and cr['reviews'] >= 10:
            ...
            if ratio >= 1.5:
                asymmetries.append({
                    ...
                    'confidence': min(0.65, 0.5 + (ratio / 50)),
                })
                break

    # === ASYMMETRY 2: Search Visibility Gap ===
    if subject_search_position is None:
        ...
        if top_comp:
            asymmetries.append({
                'category': 'GeographicVisibilityGap',
    ...
    # === ASYMMETRY 3: Authority Gap ===
    for ca in comp_authorities:
        if ca['total'] > subject_authority['total'] and ca['total'] >= 2:
            asymmetries.append({
                'category': 'AuthorityValidationGap',
                'subject_metric': f'{subject_authority["total"]} authority markers',
                'competitor_metric': f'{ca["name"]}: {ca["total"]} authority markers ({ca["awards"]} awards, {ca["media"]} media)',
                'metric_source': 'Google Search (awards + media)',
    ...
    # === ASYMMETRY 4: Review Platform Surface Gap ===
    platforms_checked = ['Google Maps', 'ProductReview', 'Facebook', 'Glassdoor', 'Indeed']
    platforms_found = []
    if subject_reviews['reviews'] > 0:
        platforms_found.append('Google Maps')
    for platform, query_suffix in [('ProductReview', f'{subject_name} site:productreview.com.au'),
                                    ('Facebook', f'{subject_name} {location} facebook reviews')]:
        results = await _serper(query_suffix, num=3)
    ...
    if len(absent) >= 2:
        asymmetries.append({
            'category': 'StaffSentimentSurfaceGap',
            'subject_metric': f'Absent from: {", ".join(absent[:3])}',
            'competitor_metric': 'Competitors present on multiple review platforms',
    ...
    # === ASYMMETRY 5: Geographic Coverage Gap ===
    suburb_count = len(subject_structure.get('suburbs_detected', []))
    if suburb_count < 3 and subject_structure.get('structure') in ('SingleLocationService', 'MultiLocationService'):
        asymmetries.append({
            'category': 'GeographicVisibilityGap',
            'subject_metric': f'{suburb_count} geographic mentions on website',
            'competitor_metric': 'Service area coverage typically 5+ suburbs',
            'metric_source': 'Website content analysis',
            'differential_ratio': f'{suburb_count}/5 coverage',
            'structural_implication': 'Geographic Visibility Compression',
            'confidence': 0.48,
        })

    # === ASYMMETRY 6: Rating Quality Gap ===
    if subject_reviews.get('rating'):
        for cr in comp_reviews:
            if cr.get('rating') and cr['rating'] > subject_reviews['rating'] and cr['reviews'] >= 20:
                asymmetries.append({
                    'category': 'ReviewDensityGap',
    ...
    # === VALIDATION: reject asymmetries with missing fields ===
    validated = []
    for a in asymmetries:
        ...
        if all(a.get(f) for f in required):
            validated.append(a)
