...
class TestCoreModuleIntegrity:
    ...
    def test_main_route_modules_loaded(self):
        ...
        assert r2.status_code == 403  # auth required but route exists
        
        # profile router
        r3 = requests.get(f"{BASE_URL}/api/business-profile")
        assert r3.status_code == 403  # auth required but route exists
        
        # admin router
        r4 = requests.get(f"{BASE_URL}/api/admin/prompts")
        assert r4.status_code == 403  # auth required but route exists
        
        # soundboard router
        r5 = requests.get(f"{BASE_URL}/api/soundboard/conversations")
        assert r5.status_code == 403  # auth required but route exists
