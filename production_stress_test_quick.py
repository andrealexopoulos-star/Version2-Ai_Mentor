...
2) Execute 25 SoundBoard chat runs (reduced for time)
...
class ProductionStressTest:
    ...
    def soundboard_chat_single_run(self, run_id: int, prompt: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Try up to 2 times for retries on rate limits
            for attempt in range(2):
                try:
                    response = self.session.post(chat_url, json=chat_data, timeout=30)
                    ...
                    if response.status_code == 429:
                        ...
                        self.log_result("SOUNDBOARD", f"Run {run_id} - Rate limited, retrying in {retry_after}s")
    ...
    def soundboard_stress_test(self, num_runs: int = 25) -> None:
        """Execute soundboard chat runs"""
        self.log_result("SOUNDBOARD_STRESS", f"Starting {num_runs} chat runs...")
        ...
        # Execute runs sequentially to avoid overwhelming server
        for i, prompt in enumerate(prompts, 1):
            result = self.soundboard_chat_single_run(i, prompt)
            self._process_soundboard_result(result)
            
            if i % 5 == 0:  # Progress update every 5 runs
                progress = (i / num_runs) * 100
                self.log_result("SOUNDBOARD_STRESS", f"Progress: {progress:.0f}% ({i}/{num_runs})")
            
            # Small delay between requests
            time.sleep(0.5)
        
        total_time = time.time() - start_time
        self.log_result("SOUNDBOARD_STRESS", f"Completed {num_runs} runs in {total_time:.2f}s")
    ...
    def test_conversation_persistence(self) -> None:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                conversation_ids = self.results['conversation_persistence']['conversation_ids'][:3]  # Test first 3
    ...
    def generate_defect_report(self) -> None:
        ...
        if soundboard['rate_limited_runs'] > 5:
            defects.append({
                'severity': 'MEDIUM',
    ...
    def run_full_test(self) -> Dict[str, Any]:
        ...
        self.log_result("MAIN", "🚀 Starting Production Backend Stress Validation (Quick Version)")
        ...
        # Step 2: Soundboard stress test (25 runs for time constraints)
        self.soundboard_stress_test(25)
...
def main():
    ...
    if auth['success']:
        ...
        print(f"  Response Time: {auth['response_time']:.2f}s")
    else:
    ...
    print(f"\nSoundboard Chat ({soundboard['total_runs']} runs):")
    ...
    print(f"  Status Codes: {soundboard['status_codes']}")
    
    if soundboard['response_times']:
        avg_time = statistics.mean(soundboard['response_times'])
        min_time = min(soundboard['response_times'])
        max_time = max(soundboard['response_times'])
    ...
    if advisor_feed['critical_failures']:
        print(f"  Failed Endpoints:")
        for failure in advisor_feed['critical_failures']:
            print(f"    - {failure}")
